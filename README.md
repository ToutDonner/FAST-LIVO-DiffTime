...
* 需要获取相机曝光时间和传输时间，补偿给图像。获取相机曝光时间和传输时间，补偿给图像。
* 我选择了在视觉部分只更新Bias，camera和lidar不同步触发的话好像不能更新state。
...
* 更新了大恒相机的[驱动](https://github.com/ToutDonner/GalaxyCameraDriver)，里面有估算传输时间的代码
...
* [ ] 测试雷达退化环境
* [ ] iVox替换iKdtree (因为ikdtree长时间运行内存会持续上涨)
