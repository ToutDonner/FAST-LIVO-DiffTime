    ...
    float LidarSelector::UpdateState(cv::Mat img, float total_residual, int level)
    {
        ...
        for (int iteration = 0; iteration < NUM_MAX_ITERATIONS; iteration++)
        {
            ...
            for (i = 0; i < sub_sparse_map->index.size(); i++)
            {
                ...
                for (int x = 0; x < patch_size; x++)
                {
                    ...
                    for (int y = 0; y < patch_size; ++y, img_ptr += scale)
                    {
                        // if((level==2 && iteration==0) || (level==1 && iteration==0) || level==0)
                        //{
                        float du = 0.5f * ((w_ref_tl * img_ptr[scale] + w_ref_tr * img_ptr[scale * 2] + w_ref_bl * img_ptr[scale * width + scale] + w_ref_br * img_ptr[scale * width + scale * 2]) - (w_ref_tl * img_ptr[-scale] + w_ref_tr * img_ptr[0] + w_ref_bl * img_ptr[scale * width - scale] + w_ref_br * img_ptr[scale * width]));
                        float dv = 0.5f * ((w_ref_tl * img_ptr[scale * width] + w_ref_tr * img_ptr[scale + scale * width] + w_ref_bl * img_ptr[width * scale * 2] + w_ref_br * img_ptr[width * scale * 2 + scale]) - (w_ref_tl * img_ptr[-scale * width] + w_ref_tr * img_ptr[-scale * width + scale] + w_ref_bl * img_ptr[0] + w_ref_br * img_ptr[scale]));
                        Jimg << du, dv;
                        Jimg = Jimg * (1.0 / scale);
            ...
            if (error <= last_error)
            {
                 ...
                 *
                 */
                // cout << "[vec:]" << vec.block<3, 1>(9, 0).transpose() << "," << vec.block<3, 1>(12, 0).transpose() << endl;
                // cout << "[solution:]" << solution.block<3, 1>(9, 0).transpose() << "," << solution.block<3, 1>(12, 0).transpose() << endl;
